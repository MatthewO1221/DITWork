...
int WINAPI WinMain(HINSTANCE instanceH, HINSTANCE prevInstanceH, LPSTR command_line, int show)
{
	...
	if (worldEditMode)
	{
		...
		for (char c = 'A'; c < 'H'; ++c)
		{
			...
			for (int i = 1; i < 21; ++i)
			{
				...
				sprintf_s(buffer, sizeof(buffer), "./Levels Old/%c%d.lvl", c, i);
	...
	// Define all room connections.
	AreaConnection allAreaConnections[MAX_AREA_COUNT];

	for (int i = 0; i < MAX_AREA_COUNT; ++i)
	{
		AreaConnection newArea =
		{
			.name = '\0'
		};
 		strcpy_s(newArea.name, sizeof(newArea.name), currentWorld.areas[i].areaName);
		allAreaConnections[i] = newArea;
	}

	game.currentRoom = &allAreaConnections[0];
	...
	if (LoadWorldAtPath(&currentWorld, "./Levels Old/main.wld") != 0)
	{
		SaveWorldAtPath(&currentWorld, "./Levels Old/main.wld");
	}
	...
	while (isGameRunning)
	{
		...
		ScreenToSmallTilePos(mouseX + (long)camX + (int)(TILE_SIZE_WORLD * 0.5F) - 832, -mouseY + (long)camY + (int)(TILE_SIZE_WORLD * 0.5F) + 512, &smallTileX, &smallTileY);
		...
		if (!isInEditMode)
		{
			...
			{
				EnterRoom(&game, &currentLevel, BOTTOM_LEFT);
			}
			else if (AEInputCheckTriggered(VK_RIGHT))
			{
				EnterRoom(&game, &currentLevel, BOTTOM_RIGHT);
			}

			if (player->x < 0 ||
				player->x > currentLevel.tilemap->width ||
				player->y < 0 ||
				player->y > currentLevel.tilemap->height)
			{
				enum Direction playerDirection;

				if ((player->x < 0 && player->y < currentLevel.tilemap->height / 2) ||
					(player->y < 0 && player->x < currentLevel.tilemap->width / 2))
				{
					playerDirection = BOTTOM_LEFT;
				}
				else if ((player->x > currentLevel.tilemap->width && player->y > currentLevel.tilemap->height / 2) ||
						 (player->y > currentLevel.tilemap->height && player->x > currentLevel.tilemap->width / 2))
				{
					playerDirection = TOP_RIGHT;
				}
				else if ((player->x < 0 && player->y > currentLevel.tilemap->height / 2) ||
						 (player->y > currentLevel.tilemap->height && player->x < currentLevel.tilemap->width / 2))
				{
					playerDirection = TOP_LEFT;
				}
				else
				{
					playerDirection = BOTTOM_RIGHT;
				}

				ShutdownPlaymodeLevel(&game);
				EnterRoom(&game, &currentLevel, playerDirection);
				InitPlaymodeLevel(&game);
			}
		}
...
void UIInputs(Tile** selectedTile, GameplayInstance* game, Level* currentLevel, World* currentWorld, long mouseX, long mouseY, unsigned char tileX, unsigned char tileY, unsigned int smallX, unsigned int smallY, float* camX, float* camY, unsigned char isDraggingBorder, unsigned char *isInWorldEditMode, unsigned char* isUnsaved)
{
	...
	UNREFERENCED_PARAMETER(smallX);
	UNREFERENCED_PARAMETER(smallY);
	...
	if (AEInputCheckCurr(VK_CONTROL))
	{
		...
		if (AEInputCheckTriggered('N'))
		{
			...
			currentWorld->areas[currentWorld->areaCount].width = MIN_AREA_WIDTH;
			currentWorld->areas[currentWorld->areaCount].height = MIN_AREA_HEIGHT;
			currentWorld->areas[currentWorld->areaCount].x = 1000;
			currentWorld->areas[currentWorld->areaCount].y = 1000;
