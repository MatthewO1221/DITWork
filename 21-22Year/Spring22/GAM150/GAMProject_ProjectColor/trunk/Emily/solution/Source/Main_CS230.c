...
void DrawBoundingBoxes(AABB* aabbs, int countX, int countY, AEGfxVertexList* mesh, AEGfxTexture* texture)
{
	for (int y = 0; y < countY; ++y)
	{
		for (int x = 0; x < countX; ++x)
		{
			...
			// Set the position.
			AEGfxSetFullTransform(aabbs[y * countX + x]->left, aabbs[y * countX + x]->bottom, 0, (aabbs[y * countX + x]->right - aabbs[y * countX + x]->left) / TILE_SIZE, (aabbs[y * countX + x]->top - aabbs[y * countX + x]->bottom) / TILE_SIZE);
