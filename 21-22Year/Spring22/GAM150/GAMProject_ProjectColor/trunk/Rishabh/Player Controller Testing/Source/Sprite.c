...
void SpriteDraw(const Sprite* sprite, TransformPtr transform)
{
	...
	AEGfxSetFullTransform(TransformGetTranslation(transform)->x, TransformGetTranslation(transform)->y, AERadToDeg(TransformGetRotation(transform)), TransformGetScale(transform)->x, TransformGetScale(transform)->y);
...
void SpriteSetFrame(SpritePtr sprite, unsigned int frameIndex)
{
	if (frameIndex <= SpriteSourceGetFrameCount(sprite->spriteSource))
	{
		...
		TraceMessage("SpriteSetFrame: frame index = %i", frameIndex);
	}
}
