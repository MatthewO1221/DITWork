...
// for example:
// distribution {1/4,1/4,1/4,1/4} creates a tree with nodes with children 0,1,2,3 equally possible
// distribution {0,1} creates degenerate binary tree with all nodes having 1 child = linked list
// distribution {0,0,1} creates a complete binary tree 
...
// 555 is not in this tree, so return value should be nullptr
template< typename SearchFunction >
void test_search_0_goal( unsigned tree_size, int branching_factor, SearchFunction funct )
{
    // std::cout << "-------- " << __func__ << " --------\n";
    ...
    auto test_search = [pRoot,val,funct]( ) {
        // for debugging
//        print_dot( pRoot, 
//                std::cout, 
...
// just to draw a random tree
void test18() 
{ 
//    std::vector<double> dist ( 3, 1.0/3 ); // 4 times 1/4
    std::vector<double> dist;
    dist.push_back( 0.1 );
    dist.push_back( 0.3 );
    dist.push_back( 0.6 );
    std::vector< std::tuple< TNode<int>*, int > > nodes = createRandomTree( 40, 1, 100, dist );
    ...
    print_dot( pRoot, 
            ...
            [](TNode<int> const * p_node)->int { return p_node->data; },
            [](TNode<int> const * p_node)->std::vector<TNode<int>*> { return p_node->children; },
            [](TNode<int> const *       )->bool { return false; } // p_node->data<50 and p_node->data>20; } // do not mark any
            );
}
